...
Bs_figure_5a_data<-process_data(figure_5a_data, "B")
...
B_drop_spear<-plot_results("\nB lymphocytes", "Spearman's Rho", " ", FALSE, Bs_figure_5a_data, "spearmans_results", "max_spearmans_error", "min_spearmans_error")
B_drop_NRMSE<-plot_results("", "NRMSE","Threshold % Dropouts", FALSE,Bs_figure_5a_data, "nrmse_results", "max_nrmse_error", "min_nrmse_error")
...
#Arrange graphs
Figure5a<-ggarrange(ggarrange(B_drop_spear + ylim(0.6,1), B_drop_NRMSE + ylim(0,50), nrow=2, ncol=1),  ggarrange(ES_drop_spear + ylim(0.6,1), ES_drop_NRMSE + ylim(0,50),nrow=2, ncol=1), leg,                                        
                    nrow = 1,
...
B_sim_cor_expr_zeros<-plot_cor_expr_zeros(Figure_5b_data, "simulated_B", " ", "% Zeros", "Simulated\nB lymphocytes", FALSE)
B_real_cor_expr<-plot_cor_expr_zeros(Figure_5b_data, "real_B", "log2(counts + 1)", "% Zeros", "Real\nB lymphocytes", FALSE)
...
Figure5b<-ggarrange(ggarrange(B_sim_cor_expr_zeros, ES_sim_cor_expr_zeros, B_real_cor_expr, ES_real_cor_expr_zeros, ncol=2, nrow=2), leg_cor, ncol = 2, widths=c(4,1))
